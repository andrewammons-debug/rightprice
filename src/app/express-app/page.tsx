...
import { ChevronLeft, Printer, Download, Share2, Loader2, Pencil } from "lucide-react";
...
export default function GenericExpressApp() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`Express_App_${lenderName || "Lender"}_${form.name || "Applicant"}.pdf`);
    } catch (e) {
      console.error(e);
    } finally {
      setBusy(false);
  ...
  const displayLender = lenderName.trim() || "LENDER NAME";
    ...
    <div className="bg-slate-950 min-h-screen text-white font-sans print:bg-white print:min-h-0">
      <style>{`
        @media print {
          ...
          .lender-edit-btn { display: none !important; }
        }
        .hide-buttons-for-pdf .no-print { display: none !important; }
        .hide-buttons-for-pdf .lender-edit-btn { display: none !important; }
      `}</style>
      ...
      <nav className="no-print bg-slate-900 fixed top-0 w-full z-50 flex justify-between items-center px-4 md:px-10 py-4 border-b border-white/5 shadow-2xl">
        ...
        <div className="text-lg font-black tracking-tighter text-white uppercase italic">Express App — Generic Lender</div>
      ...
      <main className="pt-24 pb-16 px-4 print:pt-0 print:pb-0 print:px-0">
        ...
        >
          ...
          <div className="flex justify-between items-start border-b-2 border-black pb-1 mb-1 px-2 pt-2">
            ...
            {/* ── Editable Lender Header ── */}
            <div className="border-2 border-black px-3 py-1.5 text-center leading-none min-w-[140px] relative">
              {editingLender ? (
                <div className="flex flex-col items-center gap-1">
                  <input
                    ...
                    className="text-[12px] font-black text-center w-full bg-transparent outline-none border-b border-black text-black placeholder-black/30"
                  />
                  <span className="text-[6px] text-black/50 uppercase tracking-widest">Press Enter to confirm</span>
                </div>
              ) : (
                <div
                  className="cursor-pointer group"
                  onClick={() => setEditingLender(true)}
                >
                  <div className={`text-[14px] font-black italic leading-tight ${!lenderName ? "text-black/30" : "text-black"}`}>
                    {displayLender}
                  </div>
                  <div className="lender-edit-btn mt-0.5 flex items-center justify-center gap-1 text-[6px] font-bold uppercase text-black/30 group-hover:text-black/60 transition-colors">
          ...
          <div className="px-2 py-1 text-[6.5px] font-medium text-black leading-snug">
            <p>
              I hereby authorize {displayLender} to request my credit report from a credit reporting agency.
            </p>
            <p className="mt-0.5">
              Autorizo a {displayLender} que solicite mi reporte de crédito de una agencia de crédito.
            </p>
          </div>
        </div>
      </main>
