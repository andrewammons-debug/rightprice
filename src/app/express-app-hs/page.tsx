...
import { ChevronLeft, Printer, Download, Share2, Loader2, RotateCcw } from "lucide-react";
...
// Each Cell stretches to full row height and adds bottom + optional right border
...
export default function HeritageSouthExpressApp() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`Express_App_HS_${d.name || "Applicant"}.pdf`);
  ...
  return (
    <div className="bg-slate-950 min-h-screen font-sans print:bg-white">
      ...
      <nav className="no-print bg-slate-900 fixed top-0 w-full z-50 flex justify-between items-center px-4 md:px-8 py-3 border-b border-white/5 shadow-2xl">
        ...
        <div className="text-base font-black tracking-tighter text-white uppercase italic">Heritage South — Express App</div>
      ...
      <main className="pt-16 pb-10 px-4 flex justify-center print:pt-0 print:pb-0 print:px-0">
        <div ref={ref} className="paper-form bg-white text-black border-2 border-black"
          ...
          <div className="grid grid-cols-2" style={{ flex: 1, gridAutoRows: "1fr" }}>
            ...
            {/* Last row — no bottom border (signature row has its own top border) */}
            <Cell noBorderB>
